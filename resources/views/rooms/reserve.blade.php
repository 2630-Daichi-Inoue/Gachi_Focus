    ...
    @php
        ...
        if ($rawImage) {
            if (
                ...
                str_starts_with($rawImage, 'data:image')
            ) {
                $imgSrc = $rawImage;
    ...
    <div x-data="reservePage({
      ...
      <div class="position-absolute top-0 start-0 p-4">
        <div class="display-5 fw-bold text-white text-shadow">{{ $displayName }}</div>
            ...
            <div class="col-lg-6 position-relative">
                <img src="{{ $imgSrc }}" alt="{{ $space->name }}" class="img-fluid w-100 h-100 object-fit-cover"
                    style="min-height:420px; border-radius:12px;">

                <div class="position-absolute top-0 start-0 p-4">
                    <div class="display-5 fw-bold text-white text-shadow" style="text-shadow: 1px 1px 8px rgba(0,0,0,0.6);">
                        {{ $space->name }}
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 bg-white p-4 p-md-5">
                ...
                <form method="POST" action="{{ route('rooms.reserve.submit', ['space' => $space->id]) }}"
                    x-on:submit="injectTotal()" class="mb-4">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="date" class="form-control" x-model="date" x-on:change="recalc()"
                            min="{{ $today }}" {{-- block past dates in UI --}} required>
                    </div>
                    ...
                    <div class="row g-4 mb-3">
                        <div class="col-6">
                            ...
                            <select name="time_from" class="form-select" x-model="time_from" x-on:change="recalc()"
                                required>
                                ...
                                <div class="col-sm-6 mb-2">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" value="{{ $f }}"
                                            name="facilities[]" x-model="facilities" x-on:change="recalc()"
                                            id="f-{{ Str::slug($f) }}">
                                        <label class="form-check-label"
                                            for="f-{{ Str::slug($f) }}">{{ $f }}</label>
                                    </div>
                                </div>
                            @endforeach
                        </div>
    ...
    <script>
        ...
        function reservePage(init) {
            return {
                ...
                zeroDecimals: ['BIF', 'CLP', 'DJF', 'GNF', 'JPY', 'KMF', 'KRW', 'MGA', 'PYG', 'RWF', 'UGX', 'VND', 'VUV',
                    'XAF', 'XOF', 'XPF'
                ],
